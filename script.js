(function(){
  ...
  function updateMass(){if(!eta||!beta||!gate)return;const e=+eta.value,b=+beta.value,gated=gate.checked;$("#eta-value").textContent=e.toFixed(1);$("#beta-value").textContent=b.toFixed(1);const weights=rows.map(r=>{const ref=+r.dataset.ref,a=+r.dataset.a,g=+r.dataset.g;return ref*Math.exp(e*a+b*g*(gated?Math.sign(a):1))}),z=weights.reduce((x,y)=>x+y,0);rows.forEach((r,i)=>{const p=weights[i]/z;r.querySelector(":scope > span i").style.width=`${p*100}%`;r.querySelector("output").textContent=`${(p*100).toFixed(1)}%`});const failure=weights.reduce((s,w,i)=>s+(+rows[i].dataset.a<0?w:0),0)/z;$("#gate-readout").innerHTML=gated?`Rejected answers receive <b>${(failure*100).toFixed(1)}%</b> of the mass. Teacher support on a failure is reversed.`:`Rejected answers receive <b>${(failure*100).toFixed(1)}%</b> of the mass. A confident teacher can now reinforce a failure.`}
  ...
  $("#copy-bib")?.addEventListener("click",async e=>{try{await navigator.clipboard.writeText($("#bibtex-block").textContent.trim());const old=e.target.textContent;e.target.textContent="Copied ✓";setTimeout(()=>e.target.textContent=old,1300)}catch(_){}});
})();
